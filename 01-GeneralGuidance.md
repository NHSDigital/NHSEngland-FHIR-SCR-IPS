...
* Within the IPS, mandated sections (medications, allergies, problems) should always be included. If no data exists for these sections, a section.emptyReason **MUST** be supplied. Where data does not exist for other sections, these may be left out of the Summary. Follow the international IPS guidance [here](https://build.fhir.org/ig/HL7/fhir-ips/en/Empty-Sections-and-Missing-Data.html#empty--known-absent-sections).

* For sections of the IPS (for example Results, History of Procedures etc.) there may be extra data that's not displayed on the Summary - for example where there are more than 5 results in the source system. In these cases, the Summary SHOULD say that more data is available on the source system, but doesn’t need to say how to get hold of it (its just an indication that more data is available than being displayed).
...
* Where additional patient information is available from the Spine or other external shared records, there is no requirement for the Shared Care Record generating the summary to attempt to retrieve and incorporate this into the summary that is generated. This applied to (for example) care plans & relevant documents. However, the IPS should have an indication that a Care Plan or other relevant documents exist. The IPS does not have to say where this extra information is located, and may warn that it may not be accessible to the user.
